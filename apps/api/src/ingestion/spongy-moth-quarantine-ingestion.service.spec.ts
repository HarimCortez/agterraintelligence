...
describe("SpongyMothQuarantineIngestionService", () => {
  let service: SpongyMothQuarantineIngestionService;
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: SpongyMothQuarantineClient, useValue: quarantineClientMock },
      ],
    }).compile();
    service = moduleRef.get(SpongyMothQuarantineIngestionService);
  ...
  it("only looks at timber properties (matches CitrusQuarantineIngestionService's land-use scoping, not FireAntQuarantineIngestionService's sweep-all)", async () => {
  ...
  it("creates a medium-severity flag for a timber property in a quarantined county", async () => {
    prismaMock.property.findMany.mockResolvedValue([{ id: "prop-1", county: "Highlands" }]);
    ...
    expect(prismaMock.propertyRiskFlag.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          propertyId: "prop-1",
          riskType: "spongy_moth_quarantine",
          severity: "medium",
        }),
      }),
    );

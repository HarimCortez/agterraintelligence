...
describe("CitrusCankerIngestionService", () => {
  let service: CitrusCankerIngestionService;
  ...
  const cankerClientMock = { queryCountyStatus: jest.fn() };
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: CitrusCankerClient, useValue: cankerClientMock },
      ],
    }).compile();
    service = moduleRef.get(CitrusCankerIngestionService);
  ...
  it("skips properties that already have the flag, never querying the service for them", async () => {
    ...
    expect(cankerClientMock.queryCountyStatus).not.toHaveBeenCalled();
  ...
  it("creates a medium-severity flag for a citrus property in a quarantined county (the real, current outcome for every FL seed county)", async () => {
    prismaMock.property.findMany.mockResolvedValue([{ id: "prop-1", county: "DeSoto" }]);
    cankerClientMock.queryCountyStatus.mockResolvedValue({ status: "Active Federal Quarantine" });
    ...
    expect(prismaMock.propertyRiskFlag.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          propertyId: "prop-1",
          riskType: "citrus_canker_quarantine",
          severity: "medium",
          description: expect.stringContaining("Citrus Canker"),
        }),
      }),
    );
  ...
  it("queries each distinct county only once, even with multiple properties in it", async () => {
    ...
    expect(cankerClientMock.queryCountyStatus).toHaveBeenCalledTimes(1);
    expect(cankerClientMock.queryCountyStatus).toHaveBeenCalledWith("DeSoto");
  ...
  it("does not create a flag when the county has no active quarantine on record", async () => {
    ...
    cankerClientMock.queryCountyStatus.mockResolvedValue(null);

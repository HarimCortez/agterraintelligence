...
describe("SuddenOakDeathQuarantineIngestionService", () => {
  let service: SuddenOakDeathQuarantineIngestionService;
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: SuddenOakDeathQuarantineClient, useValue: quarantineClientMock },
      ],
    }).compile();
    service = moduleRef.get(SuddenOakDeathQuarantineIngestionService);
    ...
    expect(prismaMock.propertyRiskFlag.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          propertyId: "prop-1",
          riskType: "sudden_oak_death_quarantine",
          severity: "high",
        }),
      }),
    );

...
describe("AsianLonghornedBeetleQuarantineClient", () => {
  let client: AsianLonghornedBeetleQuarantineClient;
  const fetchMock = jest.fn();

  beforeEach(() => {
    client = new AsianLonghornedBeetleQuarantineClient();
    global.fetch = fetchMock;
    jest.clearAllMocks();
  });

  it("parses a real active quarantine hit (real shape captured live, e.g. Clermont County, OH)", async () => {
    fetchMock.mockResolvedValue({
      ok: true,
      json: async () => ({ features: [{ attributes: { Quarantine_Status: "Active Federal Quarantine" } }] }),
    });

    const result = await client.queryCountyStatus("Clermont");
  ...
  it("filters Quarantine_Status server-side to only active/modified statuses — never surfaces a real rescinded record as if it were still in effect", async () => {
    ...
    await client.queryCountyStatus("Norfolk");
  ...
  it("returns null (real, confirmed outcome, not a broken query) for a Florida county — Florida has zero real ALB quarantine rows at any status", async () => {
    fetchMock.mockResolvedValue({ ok: true, json: async () => ({ features: [] }) });

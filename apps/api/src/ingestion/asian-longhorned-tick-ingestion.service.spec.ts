...
describe("AsianLonghornedTickIngestionService", () => {
  let service: AsianLonghornedTickIngestionService;
  ...
  const tickClientMock = { queryCountyStatus: jest.fn() };
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: AsianLonghornedTickClient, useValue: tickClientMock },
      ],
    }).compile();
    service = moduleRef.get(AsianLonghornedTickIngestionService);
  ...
  it("skips properties that already have the flag, never querying the service for them", async () => {
    ...
    expect(tickClientMock.queryCountyStatus).not.toHaveBeenCalled();
  ...
  it("creates a high-severity flag for an established population", async () => {
    prismaMock.property.findMany.mockResolvedValue([{ id: "prop-1", county: "Highlands" }]);
    tickClientMock.queryCountyStatus.mockResolvedValue({ status: "established" });
    ...
    expect(prismaMock.propertyRiskFlag.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          propertyId: "prop-1",
          riskType: "asian_longhorned_tick_detected",
          severity: "high",
  ...
  it("creates a medium-severity flag for a reported-only population", async () => {
    prismaMock.property.findMany.mockResolvedValue([{ id: "prop-1", county: "Highlands" }]);
    tickClientMock.queryCountyStatus.mockResolvedValue({ status: "reported" });

    const result = await service.run();

    expect(prismaMock.propertyRiskFlag.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ severity: "medium", description: expect.stringContaining("reported") }),
      }),
    );
  ...
  it("queries each distinct county only once, even with multiple properties in it", async () => {
    ...
    expect(tickClientMock.queryCountyStatus).toHaveBeenCalledTimes(1);
    expect(tickClientMock.queryCountyStatus).toHaveBeenCalledWith("Highlands");
    expect(prismaMock.propertyRiskFlag.create).toHaveBeenCalledTimes(2);
    expect(result).toMatchObject({ recordsCreated: 2 });
  });

  it("does not create a flag when the county has no detection on record (the real, current outcome for every Florida county)", async () => {
    prismaMock.property.findMany.mockResolvedValue([{ id: "prop-1", county: "Hardee" }]);
    tickClientMock.queryCountyStatus.mockResolvedValue(null);

...
describe("EmeraldAshBorerIngestionService", () => {
  let service: EmeraldAshBorerIngestionService;
  ...
  const eabClientMock = { queryCountyStatus: jest.fn() };
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: EmeraldAshBorerClient, useValue: eabClientMock },
      ],
    }).compile();
    service = moduleRef.get(EmeraldAshBorerIngestionService);
  ...
  it("skips properties that already have the flag, never querying the service for them", async () => {
    ...
    expect(eabClientMock.queryCountyStatus).not.toHaveBeenCalled();
  ...
  it("creates a medium-severity flag for a timber property in a known-infested county", async () => {
    prismaMock.property.findMany.mockResolvedValue([{ id: "prop-1", county: "Highlands" }]);
    eabClientMock.queryCountyStatus.mockResolvedValue({ firstConfirmedYear: "2016" });
    ...
    expect(prismaMock.propertyRiskFlag.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          propertyId: "prop-1",
          riskType: "emerald_ash_borer_known_infested",
          severity: "medium",
          description: expect.stringContaining("2016"),
        }),
      }),
    );
  ...
  it("queries each distinct county only once, even with multiple properties in it", async () => {
    ...
    expect(eabClientMock.queryCountyStatus).toHaveBeenCalledTimes(1);
    expect(eabClientMock.queryCountyStatus).toHaveBeenCalledWith("Highlands");
    expect(prismaMock.propertyRiskFlag.create).toHaveBeenCalledTimes(2);
    expect(result).toMatchObject({ recordsCreated: 2 });
  });

  it("does not create a flag when the county has no confirmed infestation on record (the real, current outcome for every Florida county)", async () => {
    prismaMock.property.findMany.mockResolvedValue([{ id: "prop-1", county: "Hardee" }]);
    eabClientMock.queryCountyStatus.mockResolvedValue(null);

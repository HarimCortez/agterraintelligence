...
11. **Watchlist and Saved Search tier caps (Confirmed 2026-09-13), resolving Decision 8's tracked gap** — the PRD only says Free gets a "limited watchlist," citing a table that was never transcribed with real numbers; these are real figures, not PRD-derived: **Free — 5 watchlist properties / 3 saved searches. Basic — 25 watchlist properties / 15 saved searches. Investor tier and above — unlimited (no cap) on both.** Enforced server-side (`apps/api/src/common/entitlements/tier-limits.ts`) on creation only — re-watching an already-watched property, or updating/deleting an existing saved search, never counts against the cap, so a downgraded account is never left unable to manage what it already has, only blocked from adding more.
